...
| 2026-08-10 | Lacak IP pengirim pesan | `database/migrations/2026_08_10_000001_add_sender_ip_to_messages_table.php` (baru), `app/Models/Message.php`, `database/factories/MessageFactory.php`, `app/Http/Controllers/MessageController.php`, `resources/views/admin/messages.blade.php`, `resources/views/admin/dashboard.blade.php`, `app/Http/Controllers/AdminController.php`, `resources/views/admin/export.blade.php`, `tests/Feature/MessageStoreTest.php` (baru), `tests/Feature/AdminPagesTest.php` | Kolom baru `sender_ip` di tabel `messages` — IP pengirim ditangkap di `store()` (prioritas header `X-Forwarded-For` buat proxy/CDN, fallback `request->ip()`, divalidasi format IP). Ditampilkan di halaman admin Pesan Masuk & dashboard Pesan Terbaru, plus kolom IP di export CSV pesan. IP cuma muncul di halaman admin (auth), gak bocor ke halaman publik. |
