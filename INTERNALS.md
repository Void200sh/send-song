...
| 2026-08-06 | Fix: lagu gak muncul di admin + YouTube resolve gagal | `app/Services/YouTubeService.php`, `resources/views/admin/messages.blade.php`, `resources/views/admin/dashboard.blade.php`, `app/Http/Controllers/AdminController.php`, `routes/web.php`, `.env`, `tests/Feature/AdminPagesTest.php`, `tests/Feature/ExampleTest.php` | (1) YouTubeService: fallback scrape `ytInitialData` dari `youtube.com/results` kalau API key tidak valid — resolve lagu sekarang jalan tanpa API key. (2) Kolom Lagu di admin sekarang tampil cover + judul + artis + link YouTube/Spotify (sebelumnya cuma cek `spotify_track_id`). (3) Tombol "resolve" per-pesan buat ngisi `youtube_video_id` yang hilang. (4) Dashboard: card "Lagu Terpopuler" + "Statistik Lagu". (5) `.env` dikembalikan ke SQLite (data asli ada di `database/database.sqlite`), config cache `bootstrap/cache/config.php` dihapus biar test jalan & perubahan `.env` kepake. (6) 3 pesan demo dipulihkan + `AdminSeeder` (admin@skanida.com / skanida1968). |
| 2026-08-07 | Sidebar admin: 3 fitur baru (total 6 menu) | `app/Http/Controllers/AdminController.php`, `routes/web.php`, `resources/views/admin/layouts/app.blade.php`, `resources/views/admin/songs.blade.php` (baru), `resources/views/admin/kelas.blade.php` (baru), `resources/views/admin/export.blade.php` (baru), `tests/Feature/AdminPagesTest.php` | Menu baru: **Lagu** (perpustakaan lagu unik + tombol putar YouTube/Spotify, filter search/kelas, paged), **Kelas** (rekap pesan/lagu per kelas, link ke pesan kelas itu), **Export Data** (download CSV pesan & CSV lagu, opsi rentang tanggal, BOM UTF-8 biar Excel bener). Sidebar jadi: Dashboard, Pesan Masuk, Lagu, Kelas, Export Data, Lihat Website. |
| 2026-08-10 | Lacak IP pengirim pesan | `app/Services/SpamDetectionService.php`, `app/Models/Message.php`, `database/factories/MessageFactory.php`, `app/Http/Controllers/MessageController.php`, `resources/views/admin/messages.blade.php`, `resources/views/admin/dashboard.blade.php`, `app/Http/Controllers/AdminController.php`, `resources/views/admin/export.blade.php`, `tests/Feature/MessageStoreTest.php` (baru), `tests/Feature/AdminPagesTest.php` | IP pengirim disimpan di kolom `ip_address` (hasil merge dengan fitur anti-spam dari origin/main yang juga punya kolom ini). Penangkapan IP ditingkatkan: prioritas header `X-Forwarded-For` buat proxy/CDN, fallback `request->ip()`, divalidasi format IP — di `SpamDetectionService::identity()`. Ditampilkan di halaman admin Pesan Masuk & dashboard Pesan Terbaru, plus kolom IP di export CSV pesan. IP cuma muncul di halaman admin (auth), gak bocor ke halaman publik. |
